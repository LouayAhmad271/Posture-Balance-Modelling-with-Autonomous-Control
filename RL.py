...
Complete stabilized GAIL implementation for human balance modeling.
Includes:
 - Survival-based early stopping
 - Spectrum / PCA / t-SNE comparison metrics between expert and agent
 - All stabilization features, enhanced metrics, and comprehensive evaluation.
 - CSV saving for agent and expert trajectories with exact coordinates
"""
...
# Survival early stopping params
...
def train_gail_stable(env, expert_obs, expert_acts, expert_seqs, num_iters=TOTAL_ITERS, steps_per_iter=STEPS_PER_ITER, return_metrics=False, save_every=50, checkpoint_dir="checkpoints_stable"):
    ...
    for it in range(num_iters):
        ...
        # Check for tie or marginal improvement in survival
    ...
    try:
        ...
        try:
            if feat_ex.shape[0] > 0 and feat_ag.shape[0] > 0:
                ...
                if proj.shape[1] >= 2:
                    ...
                    plt.scatter(proj_ex[:,0], proj_ex[:,1], label='expert', alpha=0.7)
                    plt.scatter(proj_ag[:,0], proj_ag[:,1], label='agent', alpha=0.7)
        ...
        try:
            if feat_ex.shape[0] > 2 and feat_ag.shape[0] > 2:
                ...
                plt.figure(figsize=(6,5))
                plt.scatter(emb_ex[:,0], emb_ex[:,1], label='expert', alpha=0.7)
                plt.scatter(emb_ag[:,0], emb_ag[:,1], label='agent', alpha=0.7)
        ...
        print("  - enhanced_comparison_metrics.json")
